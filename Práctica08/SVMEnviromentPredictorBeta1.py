...
dfTraining = pd.read_csv("datasetProyectoTesting.csv")
dfTesting = pd.read_csv("datasetProyectoTraining.csv")
dfReal = pd.read_csv("datasetProyectoReal.csv")

 
X_train = dfTraining[["edad","estado","hospital"]]
y_train=dfTraining.cantidad

X_testing = dfTesting[["edad","estado","hospital"]]
y_testing = dfTesting.cantidad

X_real = dfReal[["edad","estado","hospital"]]
y_real = dfReal.cantidad
...
#         
# 
# res3 = rbf_svc_tunning.predict(X_real)
# index = 0
# for element in res3:
#     error=(abs((element-y_real[index]))/y_real[index])*100
#     print('Predicted Value: ', element, ' Real value: ', y_real[index], " % Error: ", error)
#     index=index+1
# print("---------------------------------------------")
# 
# import csv
# pathCsvfile="./outRealTesting3.csv"
# with open(pathCsvfile, 'w', encoding='utf-8', newline='') as csvFile:
#     fieldnames = ['predictedValue', 'RealValue','ErrorPercent']
#     writer = csv.DictWriter(csvFile, fieldnames=fieldnames) 
#     writer.writeheader() 
...
#         error=(abs((element-y_real[index]))/y_real[index])*100
#         print('Predicted Value: ', element, ' Real value: ', y_real[index], " % Error: ", error)
#         writer.writerow({'predictedValue': "'"+format(element)+"'", 'RealValue': "'"+format(y_real[index])+"'", 'ErrorPercent': "'"+format(error)+"'"})
#         index=index+1
# =============================================================================
